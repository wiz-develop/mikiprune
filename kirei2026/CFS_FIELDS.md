...
| 「みる・きく」の表示方法 | `combine_see_listen` | セレクト（個別表示／合同表示） |
...
タイムスケジュールは1行に1件、`10:15〜10:30｜素肌感を活かすメイク` の形式で入力します。「合同表示」を選ぶと、「みる」と「きく」を時刻順にまとめ、各内容の先頭に種別を表示します。会場案内は各会場の見出しを開くと、フロアマップとタイムスケジュールを表示します。
